...
/// Streaming version of the comprehensive/advanced scan.
pub async fn advanced_nmap_scan_stream(
    target: &str,
    timing: Option<&str>,
    scan_type: Option<&str>,
    ports: Option<&str>,
    ...
    flag_a: bool,
    stealth_options: Option<&Value>,
) -> Result<mpsc::Receiver<NmapStreamEvent>> {
    let mut body = json!({
        "target": target
    });
    ...
    nmap::advanced_scan_stream(&body).await
}
...
pub async fn quick_scan_stream(target: &str, scan_type: &str, timing: &str) -> Result<mpsc::Receiver<NmapStreamEvent>> {
    let body = match scan_type {
        "ping_sweep" => json!({
            "target": target,
            "timing": timing,
...
pub async fn stealth_scan_stream(
    target: &str,
    stealth_level: &str,
    scan_type: &str,
    use_decoys: bool,
...
pub async fn comprehensive_scan_stream(target: &str, include_vuln: bool) -> Result<mpsc::Receiver<NmapStreamEvent>> {
    let body = json!({
        "target": target,
        "timing": "T3",
        "scan_type": "tcp_connect",
...
pub async fn network_discovery_stream(subnet: &str, timing: &str) -> Result<mpsc::Receiver<NmapStreamEvent>> {
    let body = json!({
        "target": subnet,
        "timing": timing,
        "scan_type": "ping"
